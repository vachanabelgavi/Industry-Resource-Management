...
public class SearchSupplier extends JFrame implements ActionListener {
	...
	public SearchSupplier() {
		...
		JLabel lblEmployeeId = new JLabel("Supplier ID :");
		...
		btnAdd.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				try{
					...
					PreparedStatement st=con.prepareStatement("select count(*) from supplier");
					...
					try {
							SupplierDetails frame = new SupplierDetails(x+1);
							frame.setVisible(true);
						}
					catch (Exception e) {
							e.printStackTrace();
		...
		textField_1.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				try{
					...
					String query= "select * from supplier where lower(fname) like lower('"+s+"%')";
					Class.forName("oracle.jdbc.driver.OracleDriver"); 
					Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","system","9448497388Vk");  
					PreparedStatement pst = con.prepareStatement(query);
					rs=pst.executeQuery();
		...
		btnNext.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				...
				try{
					...
					Class.forName("oracle.jdbc.driver.OracleDriver"); 
					Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","system","9448497388Vk");  
					PreparedStatement st=con.prepareStatement("select * from supplier where supl_id=?");
					st.setString(1,id);	
					//System.out.println(id);
					ResultSet rs=st.executeQuery();  
					if(rs.next()){
						...
						try {
									SupplierDetails frame = new SupplierDetails(Id,fname,lname,ph,addr);
									frame.setVisible(true);
								} catch (Exception e) {
									e.printStackTrace();
								}
