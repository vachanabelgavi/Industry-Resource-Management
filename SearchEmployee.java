		...
		btnAdd.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ae){
				try{
					...
					PreparedStatement st=con.prepareStatement("select count(*) from employee");
					...
					try {
							EmployDetails frame = new EmployDetails(x+1);
							frame.setVisible(true);
						}
					catch (Exception e) {
							e.printStackTrace();
		...
		textField_1.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				try{
					...
					String query= "select * from employee where lower(first_name) like lower('"+s+"%')";
					Class.forName("oracle.jdbc.driver.OracleDriver"); 
					Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:orcl","system","9448497388Vk");  
					PreparedStatement pst = con.prepareStatement(query);
					rs=pst.executeQuery();
			...
			public void actionPerformed(ActionEvent ae){
				...
				try{
					...
					if(rs.next()){
						...
						try {
									EmployDetails frame = new EmployDetails(Id,fname,lname,add,ph,gen,days);
									frame.setVisible(true);
							} catch (Exception e) {
								e.printStackTrace();
							}
